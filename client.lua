					...
					if IsControlJustReleased(0, 38) then -- E key
						...
						TriggerEvent("mythic_progbar:client:progress", {
							...
							if not status and not IsEntityDead(GetPlayerPed(-1)) then
                                exports['okokNotify']:Alert("Bank Robbery", "You have robbed the bank successfully for $" .. config.amount .. "!", 3000, 'success')
								TriggerServerEvent('PrintBR:PrintMessage', config.robberySuccess)
								TriggerServerEvent("Bank:Finished")
							else
                                exports['okokNotify']:Alert("Bank Robbery", "Bank robbery failed!", 3000, 'error')
				...
				if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, ammunationcoords.x, ammunationcoords.y, ammunationcoords.z) < 5.0 then
					...
					if IsControlJustReleased(0, 38) then -- E
						...
						TriggerEvent("mythic_progbar:client:progress", {
							...
							if not status and not IsEntityDead(GetPlayerPed(-1)) then
                                exports['okokNotify']:Alert("Ammunation Robbery", "You have robbed the store successfully for $" .. config.storeAmount .. "!", 3000, 'success')
								TriggerServerEvent('PrintBR:PrintMessage', config.robberyStoreSuccess)
								TriggerServerEvent("Store:Finished")
							else
                                exports['okokNotify']:Alert("Ammunation Robbery", "You have robbed the bank successfully for $" .. config.amount .. "!", 3000, 'success')
				...
				if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, shopcoords.x, shopcoords.y, shopcoords.z) < 5.0 then
					...
					if IsControlJustReleased(0, 38) then -- E
						...
						TriggerEvent("mythic_progbar:client:progress", {
							...
							if not status and not IsEntityDead(GetPlayerPed(-1)) then
                                exports['okokNotify']:Alert("24/7 Robbery:", "You have robbed the store successfully for $" .. config.storeAmount .. "!", 3000, 'success')
								TriggerServerEvent('PrintBR:PrintMessage', config.robberyStoreSuccess)
								TriggerServerEvent("Store:Finished")

							else
                                exports['okokNotify']:Alert("24/7 Robbery:", "The store robbery has failed.", 3000, 'error')
								TriggerServerEvent('PrintBR:PrintMessage', config.robberyStoreFailed)
							end
						end)
				...
				if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, ltdcoords.x, ltdcoords.y, ltdcoords.z) < 5.0 then
					...
					if IsControlJustReleased(0, 38) then -- E
						...
						TriggerEvent("mythic_progbar:client:progress", {
							...
							if not status and not IsEntityDead(GetPlayerPed(-1)) then
                            exports['okokNotify']:Alert("LTD Gas Station Robbery", "You have robbed the store successfully for $" .. config.storeAmount .. "!", 3000, 'success')
								TriggerServerEvent('PrintBR:PrintMessage', config.robberyStoreSuccess)
								TriggerServerEvent("Store:Finished")
							else
                            exports['okokNotify']:Alert("LTD Gas Station Robbery", "The robbery has failed!", 3000, 'error')
								TriggerServerEvent('PrintBR:PrintMessage', config.robberyStoreFailed)
							end
						end)
				...
				if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, liquorcoords.x, liquorcoords.y, liquorcoords.z) < 5.0 then
					...
					if IsControlJustReleased(0, 38) then -- E
						...
						TriggerEvent("mythic_progbar:client:progress", {
							...
							if not status and not IsEntityDead(GetPlayerPed(-1)) then
                            exports['okokNotify']:Alert("Liquor Store Robbery", "You have robbed the store successfully for $" .. config.storeAmount .. "!", 3000, 'success')
								TriggerServerEvent('PrintBR:PrintMessage', config.robberyStoreSuccess)
								TriggerServerEvent("Store:Finished")
							else
                            exports['okokNotify']:Alert("Liquor Store Robbery", "The robbery has failed!", 3000, 'error')
								TriggerServerEvent('PrintBR:PrintMessage', config.robberyStoreFailed)
							end
						end)
